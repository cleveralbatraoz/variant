...
namespace details {
    ...
    union variadic_union {
        ...
        void construct_from_other(VariadicUnion &&, size_t) { throw bad_variant_access(); }
    };

    template<typename HeadType, typename... TailTypes>
    union variadic_union<true, HeadType, TailTypes...> {
        ...
        template<typename... Args>
        void
        construct(in_place_index_t<0>, Args &&... args) noexcept(std::is_nothrow_constructible_v<HeadType, Args...>) {
            new(const_cast<std::decay_t<HeadType> *>(std::addressof(head_value))) HeadType(std::forward<Args>(args)...);
        }
        ...
        void construct_from_other(VariadicUnion &&other,
            ...
            if (index == 0) {
                new(std::addressof(head_value)) HeadType(std::forward<VariadicUnion>(other).head_value);
            } else {
    ...
    template<typename HeadType, typename... TailTypes>
    union variadic_union<false, HeadType, TailTypes...> {
        constexpr variadic_union() noexcept: tail_values() {}
    ...
    template<size_t Index, bool IsTriviallyDestructible, typename... Types>
    constexpr type_alternative_t<Index, Types...> const &
    get(variadic_union<IsTriviallyDestructible, Types...> const &u) {
        if constexpr (Index == 0) {
            return u.head_value;
        } else {
            return get<Index - 1>(u.tail_values);
    ...
    constexpr type_alternative_t<Index, Types...> &&get(variadic_union<IsTriviallyDestructible, Types...> &&u) {
        if constexpr (Index == 0) {
            return std::move(u.head_value);
        } else {
            return get<Index - 1>(std::move(u.tail_values));
        }
    }
